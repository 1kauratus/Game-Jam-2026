...
# -----------------------------
# Face mask overlay (WIN state)
# -----------------------------
SHOW_FACE_MASK_ON_WIN = True
SHOW_FACE_MASK_POINTS = False  # set True to draw landmark dots used for placement
...
# -----------------------------
# Face mask helpers (uses MediaPipe FaceMesh landmarks already produced by mp.solutions.face_mesh)
# -----------------------------
# Indices are from MediaPipe Face Mesh (468 landmarks)
_LM_LEFT = 234       # left side of face (near cheek/temple)
_LM_RIGHT = 454      # right side of face (near cheek/temple)
_LM_FOREHEAD = 10    # near forehead/top
_LM_CHIN = 152       # chin
_LM_EYE_L = 33       # left eye outer corner
_LM_EYE_R = 263      # right eye outer corner

def _lm_xy(lm, w, h):
    return (lm.x * w, lm.y * h)

def build_head_mask_surface(mask_base, lms, w, h):
    """Return (mask_surface, mask_rect) or (None, None) if not possible."""
    if mask_base is None or lms is None:
        return None, None
    need = max(_LM_LEFT, _LM_RIGHT, _LM_FOREHEAD, _LM_CHIN, _LM_EYE_L, _LM_EYE_R)
    if len(lms) <= need:
        return None, None

    lx, ly = _lm_xy(lms[_LM_LEFT], w, h)
    rx, ry = _lm_xy(lms[_LM_RIGHT], w, h)
    fx, fy = _lm_xy(lms[_LM_FOREHEAD], w, h)
    cx, cy = _lm_xy(lms[_LM_CHIN], w, h)
    ex1, ey1 = _lm_xy(lms[_LM_EYE_L], w, h)
    ex2, ey2 = _lm_xy(lms[_LM_EYE_R], w, h)

    head_w = math.hypot(rx - lx, ry - ly)
    head_h = math.hypot(cx - fx, cy - fy)
    if head_w < 25 or head_h < 25:
        return None, None

    # Scale factors: tune to your artwork
    target_w = int(head_w * 1.60)
    target_h = int(head_h * 1.95)

    try:
        mask_scaled = pygame.transform.smoothscale(mask_base, (target_w, target_h))
    except Exception:
        return None, None

    # Roll angle from eye line (screen coords have +y downward => negate for pygame rotation)
    angle = -math.degrees(math.atan2(ey2 - ey1, ex2 - ex1))
    mask_rot = pygame.transform.rotozoom(mask_scaled, angle, 1.0)

    center_x = (lx + rx) / 2.0

    # Place mask slightly above the forehead so it covers the whole head
    top_y = fy - (target_h * 0.55)
    rect = mask_rot.get_rect()
    rect.center = (int(center_x), int(top_y + target_h / 2.0))

    return mask_rot, rect

def draw_face_mask_points(screen, lms, w, h):
    if lms is None:
        return
    pts = [
        (_LM_LEFT, (0, 255, 0)),
        (_LM_RIGHT, (0, 255, 0)),
        (_LM_FOREHEAD, (255, 255, 0)),
        (_LM_CHIN, (255, 255, 0)),
        (_LM_EYE_L, (0, 200, 255)),
        (_LM_EYE_R, (0, 200, 255)),
    ]
...
def main():
    ...
    # -----------------------------
    # Face mask image (shown after WIN)
    # Use a transparent PNG so only the mask shows.
    # -----------------------------
    head_mask_img = None
    try:
        head_mask_img = pygame.image.load("assets/img/headmask.png").convert_alpha()
    except Exception:
        # fallback locations
        for p in ("assets/mask/headmask.png", "assets/headmask.png", "assets/img/mask.png"):
            try:
                head_mask_img = pygame.image.load(p).convert_alpha()
                break
            except Exception:
                pass
    ...
    base_right_y = None
    base_nose_y = None
    last_face_landmarks = None  # keep latest face landmarks for WIN mask overlay
    ...
    while running:
        ...
        if state == "CALIBRATING":
            ...
            # Face mask overlay (after you win)
            if SHOW_FACE_MASK_ON_WIN and head_mask_img is not None and last_face_landmarks is not None:
                mask_surf, mask_rect = build_head_mask_surface(head_mask_img, last_face_landmarks, WIN_W, WIN_H)
                if mask_surf is not None and mask_rect is not None:
                    screen.blit(mask_surf, mask_rect)
                    if SHOW_FACE_MASK_POINTS:
                        draw_face_mask_points(screen, last_face_landmarks, WIN_W, WIN_H)
